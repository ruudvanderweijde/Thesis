...
\begin{document}
    ...
    This research is executed in 3 main steps.
    The first step is to create an $M^3$ model for a PHP program, containing various facts about the program, which is explained in section \ref{sec:m3_for_php} in more details.
    Once the $M^3$ model is constructed, constraints are extracted from the $M^3$ model which is explained in section \ref{sec:fact_extraction}.
    In the third step, in section \ref{sec:constraint_solving}, the constraints are solved and resolve the types of variables used in the program.
    The final chapter \ref{sec:annotations} explains how annotations can be included in the process to gain more precise results.
    
    \section{$M^3$ for PHP}\label{sec:m3_for_php}
    Constructing a $M^3$ for PHP is differs from construction a $M^3$ for Java.
    For Java to constructing a
    ...
    \begin{algorithm}[H]
	 ...
	 m3Collection = []\;
     \BlankLine
	 \ForAll{file $\in$ program}{
	  ast = parseUsingPhpParserAndReturnRascalAST(file)\;
   	  ast = addPublicModifierWhenNotProvided(ast)\;
	  m3 = createEmptyM3(ast)\;
      \BlankLine
	  m3 = addDeclarations(m3, ast)\;
	  ast = addScopeInformation(m3, ast)\;
	  m3 = addContainment(m3, ast)\;
	  m3 = addExtendsAndImplements(m3, ast)\;
	  m3 = addModifiers(m3, ast)\;
	  m3 = addRawDocBlocksAndAnnotations(m3, ast)\;
      \BlankLine
	  m3 = calculateAliasesFlowInsensitive(m3, ast)\;
	  m3 = calculateUsesFlowInsensitive(m3, ast)\;
	  m3Collection += m3\;
	 }
	 \BlankLine
	 projectM3 = composePhpM3(m3Collection)\;
	 \BlankLine
	 \Return projectM3\;
