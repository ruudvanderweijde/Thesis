...
% https://en.wikibooks.org/wiki/LaTeX/Linguistics
   ...
   \end{table}
	...
	In PHP the $=$ symbol is used for assignments.
	In the premiss we use $:=$ for normal assigns.
	
    \fact{assignment1}{Variable assignment}{
    ...
    \fact{ternary}{Ternary operator}{
    	The ternary operator is a conditional assignment.
    	...
    	\\
    	The ternary operator without a left hand side value (\texttt{?:}), also known as the elvis operator, returns the value of $E_1$ when $E_1$ is evaluated as \texttt{true}.
    ...
    \fact{assignment2}{Assignments resulting in integers}{
		...
		The type of the left hand side ($E_1$) is in the cases of \texttt{$\&=$} (bitwise and), \texttt{$\vert{}=$} (bitwise inclusive or), \texttt{\^{}=} (bitwise exclusive or), \texttt{$<<=$} (bitwise shift left), \texttt{$>>=$} (bitwise shift right), and \texttt{$\%=$} (modulus) always \texttt{integerType()}.
    }
    \fact{assignment3}{Assignment with string concat}{
    	When the string concat operator is used, in combination with the assignment operator (\texttt{$.=$}), the type of the left hand side ($E_1$) is always a string.
    	\\
    	About the right hand side ($E_2$) we can say that \textbf{if} the type of $E_2$ is a subtype of object, then this object should have the method \texttt{$\_\_toString()$}.
    }
    \fact{assignment4}{Assignments with division or subtraction operator}{
    	Division and subtraction assignment in PHP will always result in an integer type.
    ...
    \fact{assignment5}{Assignments resulting in numbers}{
    	The result of an assignment \texttt{$*=$} (multiplication) and \texttt{$+=$} (addition) is either a float or an integer.
    ...
    \fact{unary_numbers}{Positive and negative number}{
    	When a plus ($+$) or minus ($-$) sign is used in PHP in front of a variable, the type of the whole expression must be of numberType.
    ...
    \fact{unary_negation}{Negation operators}{
    	...
    	The type of the whole expression for normal negation operator ($!$) is boolean.
    	For the bitwise negation operator ($\sim$) the type of attached variable is either a number or a string.
	...
	\fact{binary_addition}{Addition operator}{
		The result of an addition operator will always be a number or an array.
		If the left and right hand side are both arrays, the return type will be array.
	...
	\fact{binary_sub_mul_div}{Subtraction multiplication division operators}{
		These three operators are merged together in this paragraph because they have identical behaviour.
		The result of these operations is always a number.
		The operations cannot be used if one of the sides is of type array.
		Therefore we can says that the left and right hand side cannot be of arrayType.
	}
	\fact{binary_mod_bitshift}{Modulus and bitwise shift operators}{
		The merge of modulus and bitwise shift operators seems not so obvious at first, but they have the same behaviour.
		The results of these operations is of integer type.
	}
	\fact{binary_bitwise}{Bitwise operators}{
		The results of the bitwise operators \texttt{and}, \texttt{or}, and \texttt{xor} is always of integer or string type.
		When the left and right hand side are both strings, the result of the operation is also of type string.
		In all other cases the result of this operation is a number.
		For the sake of readability we've omitted the constraints for the operators \texttt{bitwise or} (inclusive or) and \texttt{bitwise xor} (exclusive or).
	}
    ...
    %%
    \subsection{Array}
    Introduce array here...
    
    \fact{array}{Array declaration}{
    	From the array declarations (\texttt{array()} or \texttt{[]}) we can extract the constraint that they should be of any array type.
    }
    \fact{arrayAccess}{Array value fetch}{
    	From the usage of array access you cannot tell what the type of the expression is.
    	The same syntax is used to access strings.
    	We can only extract that the type should not be of object type.
    }
