...
\end{equation}
    ...
    The process of actually resolving types is called \textbf{type inference}.
    ...
    Static type checking is a process of checking the types based on the source code.
    This type checker will ensure that a program is type safe, which means that there will occor no type errors, before the program is executed.
    Dynamic type checking performs the type checking during run time.
    ...
    PHP is fully dynamically typed, which means that there are no types checked before actually running the program.
    The advantage of static types is that they can catch type errors early and can perform compiler optimisations, while dynamic type checking systems allow more flexible in their type allowance.
    The downside of static types is that they can be too restrictive, while dynamic typing allows run time type errors which can lead to hard to locate errors.
    
    \paragraph{Type inference}
    Type inference is the process of resolving the type of a variable or expression.
    In order to validate the types during type checking, we first need to know the types of variables and expressions.
