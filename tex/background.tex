...
\end{equation}
    ...
    PHP is a dynamically typed language, which means that the type of expressions cannot be resolved at compile time.
    %todo this line totally makes no sense
    The time ... because the language allows expressions to change to a different type during run-time.
    This thesis describes a way to over-approximate the run-time types, using static analysis.
   
    \textbf{Type systems} define how a set of rules are applied to types in their context.
    The type system validates the type usage with \textbf{type checking}.
    ...
    Despite the good intentions, PHP7 will support scalar type hints as provided by this tool.
