...
\begin{document}
    ...
    For the keyword \texttt{self} we will have to refer to the class it's declared in, plus all its descended classes.
...
\end{equation}
    ...
    %todo (Jurgen) I need to read about the goal of your thesis more clearly. A list of the contributions of your thesis? An design of a static type system for PHP and a type inferencer for PHP? Put this more in the beginning and then it will be more clear why we are reading about all these things.
    
    %todo add duck typing as sub part of Type Systems
    \section{Type systems}\label{sec:background_type-system}
    %todo (Jurgen) to read the first two paragraphs, better first quickly introduce the concept of types. Also, immediately distinguish static from dynamic types for PHP. Your thesis is about static over-approximations of run-time types. A type is a set of values. A nominal type in code defines a set of run-time values and the operations you can define on them. something like that in your own words?
    ...
    On one hand the system needs to determine the type of variables, on the other hand it will check the type of the variables. %todo (Jurgen) this is too vague
    ...
    %todo (Jurgen) this stuff is interesting and deserves a little more space. explain why it is relevant for your thesis.
	\\
    ...
    \\
    %todo check where type inference is used in the process
    During ... a xxx form of type inference is used to optimise the results.
    ...
    \footnotetext{http://www.icosaedro.it/phplint, July 2015}
